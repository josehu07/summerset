...
impl ServerRpcSender {
    ...
    pub fn connect<C, Fut>(
        &mut self,
        connect_fn: impl FnOnce(String) -> Fut,
        peer_addr: &String,
    ) -> Result<C, SummersetError>
    where
    ...
    pub fn connect_all<C, Fut>(
        &mut self,
        connect_fn: impl FnOnce(String) -> Fut + Send + Copy + 'static,
        peer_addrs: &Vec<String>,
    ) -> Result<HashMap<usize, C>, SummersetError>
    where
        C: Send + 'static,
        Fut: Future<Output = Result<C, tonic::transport::Error>> + Send,
    {
