...
enum PeerMsg {
    ...
    PrepareReply {
        ...
        /// start serving reads single-handedly. A better implementation now
        /// would be to use the `Guard` message to carry this information, but
        /// doing it a bit more conservatively with `PrepareReply` also works.
        accept_bar: usize,
    },
...
impl GenericReplica for MultiPaxosReplica {
    ...
    async fn new_and_setup(
        ...
        let mut transport_hub = TransportHub::new_and_setup(
            ...
            p2p_addr,
            HashMap::from([(
                0, // only one lease purpose exists in MultiPaxos
                tx_lease_msg,
            )]),
        )
        .await?;
