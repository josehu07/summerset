...
CONSTANT
    ...
    Slots,          \* set of slots (instances); 1 should be enough
    Ballots,        \* set of ballot numbers
    Shards,         \* set of numbers representing shards (columns)
    NumDataShards,  \* number of data shards needed to reconstruct
    MaxFaults       \* fault-tolerance level
...
variable msgs = {},
         ...
         lStatus = [r \in Replicas |-> [s \in Slots |-> ""]],
         rBallot = [r \in Replicas |-> -1],
         rVoted  = [r \in Replicas |-> [s \in Slots |-> [bal |-> -1,
                                                         val |-> 0,
                                                         shards |-> {}]]],
         proposed = [s \in Slots |-> {}],
         learned = [s \in Slots |-> {}];
    ...
    ValuePreparedIn(v, prPat, pBal) ==
        ...
        \/ /\ Cardinality(prPat) >= MajorityNum
           /\ \E c \in 0..(pBal-1):
                ...
                /\ IsGoodCoverageSet(
                     {pr.vShards: pr \in {prr \in prPat: prr.vVal = v}})
        \/ /\ BigEnoughUnderFaults(prPat, Replicas)
           /\ ~\E vv \in Values:
                IsGoodCoverageSet(
                  {pr.vShards: pr \in {prr \in prPat: prr.vVal = vv}})
...
\* This is the first message a leader makes after election. Think of this as a
\* Prepare message that covers infinitely many slots up to infinity.
...
\* Replicas reply with known shards for necessary recovery reconstruction.
    with m \in msgs do
...
\* Shards are assigned according to some assignment policy, from a set where
\* all reasonable assignments are considered.
    await lStatus[r][s] = "Preparing";
    with v \in Values do
        await \E MS \in SUBSET {
                m \in msgs: /\ m.type = "PrepareReply"
                            /\ m.bal = lBallot[r]}:
...
\* Such a reply will not carry actual value data in practice; just the shards
\* assignment metadata is enough for the leader to gather Acceptance Patterns.
    with m \in msgs do
        await /\ (m.type = "Accept")
              /\ (m.to = r)
              /\ (m.bal >= rBallot[r]);
...
\* BEGIN TRANSLATION (chksum(pcal) = "5ca7da7d" /\ chksum(tla) = "8b7bd479")
...
Replica(self) == \/ /\ \E b \in Ballots:
                 ...
                 \/ /\ \E m \in msgs:
                         ...
                         /\ msgs' = (msgs \cup {([type |-> "AcceptReply",
                                                  ...
                                                  shards |-> m.shards])})
                         /\ rBallot' = [rBallot EXCEPT ![self] = m.bal]
                         /\ rVoted' = [rVoted EXCEPT ![self][m.slot] = [bal |-> m.bal,
                                                                        val |-> m.val,
                                                                        shards |-> m.shards]]
