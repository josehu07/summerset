...
PAIRS_NETEM_JITTER = utils.config.PairsMap({}, default=10)
...
def plot_put_ratio_results(results, put_ratio, plots_dir, ymax=None):
    ...
    # CLIENT_GROUPS_ORDER = [1, 2, 3, 0, 4]
    if put_ratio > 0:
        CLIENT_GROUPS_ORDER = [1, 3, 0, 4]
    ...
    if put_ratio == 0:
        ...
        plt.ylim(0.0, 65.2 - 62.5)
        plt.yticks([1.0, 65.0 - 62.5], ["1x", "65x"])
    elif put_ratio == 1:
        plt.ylim(0.0, 19.0)
        plt.yticks([1.0, 9.0, 18.0], ["1x", "9x", "18x"])
    elif put_ratio == 10:
        plt.ylim(0.0, 3.6)
        plt.yticks([1.0, 2.0, 3.0], ["1x", "2x", "3x"])
    ...
    plt.ylim(0.0, 300.0)
    plt.yticks([0, 100, 200], list(map(str, [0, 100, 200])))
...
if __name__ == "__main__":
    ...
    if not args.plot and len(args.fetch) == 0:
        ...
        for host in hosts[:NUM_REPLICAS]:
